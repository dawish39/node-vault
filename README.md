...
系統由以下元件構成，各司其職，資料流單向：

```
Claude + session-wrap-ex  →  node-check（可選）  →  GAS Web App  →  Google Drive  →  同步軟體  →  Obsidian
（產出與閘控）               （獨立檢核）           （自動搬運）      （持久儲存）      （本地同步）    （導航視覺化）
```

<img width="680" height="587" alt="node-vault_圖一_裝置與同步架構" src="https://github.com/user-attachments/assets/5207068b-6682-427d-b808-4bb0170030ed" />

| 元件 | 角色 |
|---|---|
| **Claude + session-wrap-ex**[^2] | 對話收尾與閘控。人工判斷在這裡發生，確認前節點不產出。 |
| **node-check**（可選） | 節點產出後的獨立檢核。在新對話中重新驗證外部引用與內部邏輯，見 Section 4。 |
...
<img width="680" height="587" alt="node-vault_圖二_單次對話工作流" src="https://github.com/user-attachments/assets/04326a11-d3c5-4b03-9244-1128cfd302aa" />

---

## 4. node-check：節點的獨立檢核（可選）

用 AI 討論事情的時候，AI 經常會幫你查資料。問題是它查回來的東西不一定對——引用範圍縮窄、把兩篇不同的研究混在一起講、或是用很有信心的語氣說一個其實是它自己推測的結論。你在對話當下很難逐條核實，因為你正在思考主題本身，不是在做 fact-checking。

這些錯誤會跟著節點一起入庫。六個月後取回，你不會記得哪些是查證過的、哪些是當時順著 AI 的說法接受的。

node-check 是從這個問題長出來的。我手動做了很多次正確性檢核，發現每次都在重複同樣的動作：開一個新對話、把節點貼進去、請 AI 重新查一次外部引用、檢查內部邏輯有沒有矛盾。做了夠多次之後，我把這些重複的動作整理成一個結構化流程，讓它自動產出檢核用的 prompt。

### 它做什麼

在 session-wrap-ex 產出節點後，在同一個對話裡觸發 node-check。它會根據這個節點的內容，自動生成一份結構化的檢核 prompt。你把這份 prompt 複製到**一個新的對話**裡執行。

為什麼要開新對話：產出節點的對話已經有既定的思路，AI 會傾向確認自己剛才說過的東西，而不是重新審視。同時，長對話的 context 越長，AI 對中間段落的注意力越弱，檢核品質會隨對話長度下降。新對話從零開始，只看節點文字本身——既避開既有思路的干擾，也讓 AI 能把注意力集中在節點內容上。

### 檢核三階段

| 階段 | 檢查什麼 | 抓什麼問題 |
|---|---|---|
| Phase 1：外部引用查核 | 節點裡引用的研究、理論、外部來源 | AI 查錯了、引用範圍描述不精確、意譯被當成直接引用 |
| Phase 2：內部邏輯檢核 | 節點自身的推論鏈與結論 | 前提和結論之間有跳躍、同一來源被呈現為多條獨立證據、類比強度偏高 |
| Phase 3：跨節點一致性 | see_also 連結的其他節點 | 引用其他節點時是否忠於原意、宣稱的「缺口」是否真的存在 |

### 如何觸發

session-wrap-ex 產出節點後，在同一個對話裡輸入：

```
node-check / 檢核 / 幫我產檢核 prompt
```

### 什麼時候該用

**建議使用**：節點引用了外部研究或理論、包含跨領域的類比或框架遷移、核心價值在推論而非事實紀錄。

**可以跳過**：純操作紀錄（做了什麼、怎麼做的）、對照原文就能驗證的讀書筆記、時間壓力下先求入庫的情況。

不是每個節點都需要跑。判斷「要不要檢核」本身就是一次品質決策。

### 檢核結果怎麼處理

node-check 產出的是一份報告，不是自動修正。它會標出哪些地方有問題、問題是什麼，但不會直接改你的節點。要不要修、怎麼修，由你決定。
...
4. 部署為 Web App：「部署」→「新增部署」→ 類型選「網頁應用程式」→ 執行身分選「我」→ 存取權選「知道連結的任何人」→「部署」
   ...
   > ⚠️ **安全說明**：此設定不含身份驗證，知道 URL 的人即可呼叫後端。URL 請勿公開分享。個人單機使用場景下風險可接受；如有顧慮，見 Section 7 Known boundaries。
5. 複製 Web App URL，日後拖入 MD 時開啟此頁面

**驗證**：用瀏覽器打開 Web App URL，應該看到如下的上傳介面。若出現錯誤頁面，最常見原因是 Script Properties 三個 key 有空白或填錯 Drive ID。
<img width="1799" height="995" alt="image" src="https://github.com/user-attachments/assets/183a41ac-4552-48d7-8442-793aa9d7f110" />
...
### Step 3.5（可選）：安裝 node-check SKILL

如果你想在入庫前對節點做獨立檢核（見 Section 4），可以額外安裝 node-check skill：

1. 從本 repo 的 [skills/node-check/SKILL.md](https://github.com/dawish39/node-vault/blob/main/skills/node-check/SKILL.md) 下載 SKILL.md 檔案
2. 同 Step 3 的方式上傳至 Claude Skills
3. 上傳成功後在 Skills 清單裡會看到 `node-check`，確認已啟用

不裝也不影響主流程。建議先跑 session-wrap-ex 累積一些節點後，再視需求加入。
...
<img width="1151" height="859" alt="image" src="https://github.com/user-attachments/assets/a4a94059-f73d-471e-9e0d-cf1be8697d6c" />
...
**個人紀律依賴**：制度成本由個人紀律吸收（詳見 Section 5 工作流前置條件層）。多人協作場景下流程層需另行設計，尚未實作。
...
- node-check 實測數據目前為少量節點，檢核命中率與 false positive 率尚未系統性統計
- 跨 session 隔離的淨效果（去偏差增益 − 脈絡損失）未經對照驗證
- 檢核的 token 成本取決於節點的外部引用數量與 see_also 節點數量，變動範圍大
- node-check 產出的檢核 prompt 可在任何 AI 工具中執行，不限於 Claude
...
[^2]: **SKILL**：Claude 的結構化指令集。透過特定格式讓 Claude 在對話中執行固定的多步驟協議，並在指定步驟要求停止等待人工介入。SKILL 是指令層機制（非執行層強制），實務上 Claude 對 SKILL 的遵循率高，但並非絕對保證；發現未停住時請參考 Section 6 的故障排除。
